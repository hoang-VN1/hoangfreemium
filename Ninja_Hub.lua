...
-- NightHub-compatible server hop layer (keeps Bacon's original hop as first choice)
local NightHopBusy = false
local NightVisitedServers = {}
local NightHopApis = {
  SoulReaper = "https://minhnghia.pythonanywhere.com/api/SoulReaper",
  CastleRaid = "https://minhnghia.pythonanywhere.com/api/CastleRaid",
  CakePrince = "https://minhnghia.pythonanywhere.com/api/CakePrince",
  Elite = "https://minhnghia.pythonanywhere.com/api/Elite",
  TyrantOfTheSkies = "https://minhnghia.pythonanywhere.com/api/TyrantOfTheSkies",
  RipIndra = "https://minhnghia.pythonanywhere.com/api/RipIndra",
  CursedCaptain = "https://minhnghia.pythonanywhere.com/api/CursedCaptain",
  Fullmoon = "https://minhnghia.pythonanywhere.com/api/Fullmoon"
}
local NightHopDecodeMap = {
  ["!"]="a", ["@"]="b", ["P"]="c", ["$"]="d", ["M"]="e", ["V"]="f",
  ["&"]="g", ["*"]="h", ["("]="i", [")"]="j", ["-"]="k", ["_"]="l",
  ["L"]="m", ["+"]="n", ["{"]="o", ["}"]="p", ["["]="q", ["]"]="r",
  [";"]="s", [":"]="t", ["'"]="u", ["\""]="v", ["<"]="w", [">"]="x",
  ["?"]="y", ["~"]="z", ["A"]="0", ["B"]="1", ["C"]="2", ["D"]="3",
  ["E"]="4", ["F"]="5", ["G"]="6", ["H"]="7", ["I"]="8", ["J"]="9", ["S"]="-"
}
local function NightDecodeJobId(x)
  x = tostring(x or ""):gsub("^NightHub%-U3R1cGlkTmlnZ2Vy", ""):gsub("=+$", "")
  local out = {}
  for ch in x:gmatch(".") do out[#out+1] = NightHopDecodeMap[ch] or ch end
  return table.concat(out)
end

local function NightPublicServers()
...
local function NightHopPublic(lowPlayers)
  if NightHopBusy then return false end
  NightHopBusy = true
  local success = false
  task.spawn(function()
    local servers = NightPublicServers()
    if type(servers) == "table" then
      if lowPlayers then
        table.sort(servers, function(a,b) return (a.playing or math.huge) < (b.playing or math.huge) end)
      end
      for _, server in ipairs(servers) do
        local id = tostring(server.id or "")
        local playing = tonumber(server.playing) or 0
        local maxPlayers = tonumber(server.maxPlayers) or 0
        if id ~= "" and id ~= game.JobId and not NightVisitedServers[id] and playing < maxPlayers then
          NightVisitedServers[id] = true
...
function NightHopApi(name, age)
  local api = NightHopApis[name]
  if not api or NightHopBusy then return false end
  NightHopBusy = true
  task.spawn(function()
    local HttpService = game:GetService("HttpService")
    local chosenJob, minAge = nil, math.huge
    local ok, body = pcall(function() return game:HttpGet(api) end)
    if ok and type(body) == "string" then
      local ok2, response = pcall(function() return HttpService:JSONDecode(body) end)
      if ok2 and type(response) == "table" and type(response.data) == "table" and (tonumber(response.count) or #response.data) > 0 then
        for _, entry in pairs(response.data) do
          if type(entry) == "table" then
            local encoded = entry.JobId or entry.jobId or entry.JobID
            local serverAge = tonumber(entry.age) or 0
            local jobId = NightDecodeJobId(encoded)
            local maxAge = tonumber(age) or 120
            if jobId ~= "" and jobId ~= game.JobId and serverAge <= maxAge and not NightVisitedServers[jobId] and serverAge < minAge then
              minAge = serverAge
              chosenJob = jobId
            end
          end
    ...
    NightHopBusy = false
    if chosenJob then
      NightVisitedServers[chosenJob] = true
      pcall(function()
        if replicated:FindFirstChild("__ServerBrowser") then
          replicated.__ServerBrowser:InvokeServer("teleport", tostring(chosenJob))
        else
          TeleportService:TeleportToPlaceInstance(game.PlaceId, tostring(chosenJob), plr)
        end
      end)
    else
      NightHopPublic(true)
...
Hop = function()
  -- Preserve Bacon's original ServerBrowser hop when available, then fall back to public API.
  local ok = pcall(function()
    for count = math.random(1, math.random(40, 75)), 100 do
      local remote = replicated.__ServerBrowser:InvokeServer(count)
      for jobId, v in next, remote do
        if tonumber(v["Count"]) and tonumber(v["Count"]) < 12 and tostring(jobId) ~= game.JobId then
  ...
  if not ok then NightHopPublic(false) end
end

-- Optional auto-hop helpers for Bacon's existing farm toggles. These do not replace the farms.
local NightAutoHop = {
  CakePrince = false, Bones = false, Chest = false, Berry = false, Elite = false,
  Tushita = false, SoulReaper = false, RipIndra = false, Tyrant = false
}

local function NightTargetExists(name)
  local ok, result = pcall(function() return GetConnectionEnemies(name) end)
  return ok and result ~= nil
end

local function NightAddHopToggle(title, key, farmFlag, target, apiName)
  local t = Tabs.Misc:AddToggle("NightHop_" .. key, {Title = title, Description = "Use NightHub-style hop when the target is missing", Default = false})
  t:OnChanged(function(v) NightAutoHop[key] = v end)
  task.spawn(function()
    while task.wait(3) do
      if NightAutoHop[key] and _G[farmFlag] then
        local exists = false
        if type(target) == "table" then
          for _, name in ipairs(target) do
            if NightTargetExists(name) then exists = true break end
          end
        else
          exists = NightTargetExists(target)
        end
        if not exists then
          if apiName then NightHopApi(apiName, 60) else NightHopPublic(true) end
          task.wait(8)
        end
      end
    end
...
-- =========================================================
-- NINJA HUB BRANDING / INTRO
-- Author credit is taken from the original NightHub UI:
-- "by dev_hoàng@_@ | https://discord.gg/U3euTqAabv"
-- =========================================================
local NINJA_HUB_IMAGE = "rbxassetid://116164900791743"
local NINJA_HUB_AUTHOR = "by dev_hoàng@_@ | https://discord.gg/U3euTqAabv"

local function NinjaHubIntro()
    local Players = game:GetService("Players")
    local TweenService = game:GetService("TweenService")
    local CoreGui = game:GetService("CoreGui")

    local gui = Instance.new("ScreenGui")
    gui.Name = "NinjaHubIntro"
    ...
    title.Size = UDim2.fromOffset(700, 110)
    ...
    credit.Position = UDim2.fromScale(0.5, 0.57)
    credit.Size = UDim2.fromOffset(900, 32)
    credit.BackgroundTransparency = 1
    credit.Text = NINJA_HUB_AUTHOR
    credit.Font = Enum.Font.GothamMedium
    credit.TextSize = 13
    credit.TextColor3 = Color3.fromRGB(170, 175, 195)
    ...
    logo.Image = NINJA_HUB_IMAGE
    logo.ImageTransparency = 1
    logo.Parent = bg

    TweenService:Create(title, TweenInfo.new(0.85, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        TextSize = 64,
        TextTransparency = 0,
    }):Play()
    TweenService:Create(stroke, TweenInfo.new(0.85, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Transparency = 0.25,
    }):Play()
    TweenService:Create(credit, TweenInfo.new(0.7, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        TextTransparency = 0,
    }):Play()

    task.wait(1.15)

    TweenService:Create(title, TweenInfo.new(0.55, Enum.EasingStyle.Quint, Enum.EasingDirection.InOut), {
    ...
    TweenService:Create(credit, TweenInfo.new(0.4), {
        TextTransparency = 1,
    }):Play()
    ...
    TweenService:Create(logo, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
...
-- Floating Ninja Hub image button.
-- Click to toggle Fluent window (instead of sending End key)
task.spawn(function()
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
    ...
    button.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            ...
            moveConnection = UIS.InputChanged:Connect(function(changed)
    ...
    -- Chỉ một sự kiện Activated duy nhất
    button.Activated:Connect(function()
        if dragging then return end
        Window:Toggle()  -- Ẩn/hiện cửa sổ Fluent
    end)
end)
...
Tabs.Misc:AddSection("NightHub Hop / Farm Hop")
Tabs.Misc:AddButton({Title = "NightHub Hop", Description = "Server hop with Bacon fallback",Callback = function()
  Hop()
end})
Tabs.Misc:AddButton({Title = "NightHub Hop Lowest", Description = "Find a low-player public server",Callback = function()
  NightHopPublic(true)
end})
Tabs.Misc:AddButton({Title = "Hop Soul Reaper", Description = "API target hop",Callback = function() NightHopApi("SoulReaper", 60) end})
Tabs.Misc:AddButton({Title = "Hop Castle Raid", Description = "API target hop",Callback = function() NightHopApi("CastleRaid", 20) end})
Tabs.Misc:AddButton({Title = "Hop Cake Prince", Description = "API target hop",Callback = function() NightHopApi("CakePrince", 60) end})
Tabs.Misc:AddButton({Title = "Hop Elite", Description = "API target hop",Callback = function() NightHopApi("Elite", 60) end})
Tabs.Misc:AddButton({Title = "Hop Tyrant of the Skies", Description = "API target hop",Callback = function() NightHopApi("TyrantOfTheSkies", 120) end})
Tabs.Misc:AddButton({Title = "Hop Rip Indra", Description = "API target hop",Callback = function() NightHopApi("RipIndra", 120) end})
Tabs.Misc:AddButton({Title = "Hop Cursed Captain", Description = "API target hop",Callback = function() NightHopApi("CursedCaptain", 67) end})
Tabs.Misc:AddButton({Title = "Hop Full Moon", Description = "API target hop",Callback = function() NightHopApi("Fullmoon", 30) end})

NightAddHopToggle("Auto Hop Cake Prince", "CakePrince", "Auto_Cake_Prince", "Cake Prince", "CakePrince")
NightAddHopToggle("Auto Hop Bones", "Bones", "AutoFarm_Bone", {"Reborn Skeleton","Living Zombie","Demonic Soul","Posessed Mummy"}, nil)
NightAddHopToggle("Auto Hop Elite", "Elite", "FarmEliteHunt", {"Diablo","Urban","Deandre"}, "Elite")
NightAddHopToggle("Auto Hop Tushita", "Tushita", "Auto_Tushita", "Longma", nil)
NightAddHopToggle("Auto Hop Soul Reaper", "SoulReaper", "AutoHytHallow", "Soul Reaper", "SoulReaper")
NightAddHopToggle("Auto Hop Rip Indra", "RipIndra", "AutoRipIngay", {"rip_indra","rip_indra True Form"}, "RipIndra")
...
Window:SelectTab(1)
loadstring(game:HttpGet("https://raw.githubusercontent.com/hoang-VN1/Tets/refs/heads/main/Attackbyhoang"))()
